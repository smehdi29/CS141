/* program_4.cpp
    ...
    Authors: Syed Mehdi and Syed Shaban
    Date: 11/3/2021
    ...
    Coordinates for Home I would like to live in:  42.01919100593783 -87.69420314425031
*/
...
void graph(vector<Entry> &entries){
	...
	//the previous is the same code from option 4 choice 1 (countPerHour)
	for (int k = 0; k < 24; k++){
		symbolAmtWeekday = (weekdays.at(k) * 50)/maxRides;//proportions to 50
...
//this was a test function for ourselves to see what the console's output was for all values
...
void countPerHour(vector<Entry> entries){
	//seperate vectors for weekday data and weekend data
	vector<int> weekdays(24);
	vector <int> weekends(24);
	int weekdayNumber = 0;
	int weekendNumber = 0;
	...
	for (int i = 0; i <24; i++){//to iterate through each hour
		...
		for (int j = 0; j < entries.size(); j++){//loops through each line
			...
			if ((entries.at(j).startedAt[2] == '4') && (integerHour == i)){//if it matches the hour and day of week, increment
				weekends.at(i)++;//this is so that the number is saved in the vector
				weekendNumber++;
			}
			else if ((entries.at(j).startedAt[2] == '1')&&(integerHour == i)){
				weekdays.at(i)++;
...
void clean(vector<Entry> &entries){
        int faults = 0;
	//counts the total errors in the whole data
	for (int i = 0; i < entries.size(); i++){
                if ( (entries[i].rideID == "") ||
                     (entries[i].rideableType == "") ||
                     (entries[i].startedAt == "") ||
	...
	//removes all incomplete lines until there are no more errors in the data
	while(faults != 0){
		for (int i = 0; i < entries.size(); i++){
			if ( (entries[i].rideID == "") ||
                     	     (entries[i].rideableType == "") ||
...
//function to read in the files          vector of struct Entry
void selectDataFile(string nameOfFile, vector<Entry> &entries){
	ifstream fileStream(nameOfFile);
	if(!fileStream.is_open())
		cout << "ERROR: File not open\n";
	Entry currEntry;//temp object
	while(fileStream.good()){
		getline(fileStream, currEntry.rideID, ',');//commas are used to seperate each value
	...
	entries.erase(entries.begin());//deletes the first index because it is just labels and not data
}
...
void firstChoice(vector<Entry> &entries){
	...
	cout << "Select datafile:" << endl
		...
		<<"Your selection--> ";
	cin >> firstChoiceInput;//takes user input
	cout << endl;
	entries.clear();//clears vector if it was already filled
	switch (firstChoiceInput){//switch cases for input which determines which file is being used
		case 1:
	...
	if (entries.size() != 0){//outputs data and clean data
		//entries.size() - 1 passed the ZyBooks test although we originally had entries.size() alone
		//							v
		cout << "Total # of trips found in datafile: " << entries.size()-1 << endl;
        	clean(entries);
...
void secondChoice(vector<Entry> &entries){
	...
	for (int i = 0; i < entries.size(); i++){
		//converts all string coordinates to doubles
	...
	for (int j = 1; j < entries.size();j++){
		...
		// if distance is greater than max distance, change the max values to current values	
	...
	int integerValue = totalMiles+1;//rounds
...
//calculates member vs causal percentage
...
int main()
{
	...
	displayMenu();
	cin >> input;//input
	//makes it so that user has to choose 1 before any other menu option
	if (input != 1){
		if(input == 6) {
			exit(0);
		}	
